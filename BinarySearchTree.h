...
private:
	node<T>* root;
	void inOrderT(node<T>* p, T & item, int *count);
...
template <class T>
int BinarySearchTree<T>::inOrder(T &item){
	int num = 0;
	inOrderT(root, item, &num);
	return num;

}

template <class T>
void BinarySearchTree<T>::inOrderT(node<T>* p, T &item, int *count){
   if(p!=NULL){
		inOrderT(p->left, item, count);
		if(p->data < item){
			*count +=1;
			cout<<p->data;
		}
		inOrderT(p->right, item, count);
   }
...
template <class T>
T* BinarySearchTree<T>::searchTree(T& item){
	return search(root,item);;

}

template <class T>
T* BinarySearchTree<T>::search(node<T>*& ptr,T& item){
	if( ptr == NULL)
		return NULL;
